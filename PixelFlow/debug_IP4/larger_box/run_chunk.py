"""Run a contiguous chunk CONFIGS[start:end] on the given GPU.

Per config:
  - Run run_ip4 with kw on a 2-image GT sample, 128x128 box mask
  - Compute per-image PSNR (full + unobs region) / SSIM / LPIPS / HF
  - Save:  results/<name>.png (GT|Meas|recon side-by-side, 2 rows)
           results/<name>.json (kw + metrics)
           results/<name>.pt   (xf tensor, for later FID/aggregation)

Usage:
  python run_chunk.py --start 0 --end 4 --gpu 0
"""
...
def main():
    ...
    # 128x128 box at random position (deterministic seed for reproducibility)
    ...
    for i, (name, kw) in enumerate(chunk):
        gi = args.start + i
        print(f"\n[{gi+1}/{len(CONFIGS)}] {name}  he={kw['h_epsilon']!s} L={kw['num_langevin']!s}", flush=True)
        t0 = time.time()
        try:
        ...
        with torch.no_grad():
            for b in range(B):
                ...
                # PSNR in unobserved region only
                m = (1 - mask[b:b+1])
                diff2 = (r1 - g1).pow(2)
        ...
        torch.save({"xf": xfD.cpu(), "name": name, "kw": kw, "metrics": meta},
                   f"{RESULT_DIR}/{name}.pt")

        # Save 2x4 image grid (per-config visual)
