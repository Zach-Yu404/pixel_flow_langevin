...
class PixelFlowPipeline3:
    ...
    def __call__(
        ...
        if xt is None:
            ...
            assert start_stage is not None and start_T is not None, "start stage and start T should be provided!"
            assert xt.ndim == 4 and xt.shape[1] == 3, f"xt must be BCHW with C=3, got {tuple(xt.shape)}"
            assert isinstance(xt, torch.Tensor), f"xt must be torch.Tensor, but got {type(xt)}"
            assert xt.dtype == torch.float32, f"x dtype must be torch.float32, but got {xt.dtype}"
            assert type(start_stage) is int, f"start stage must be int (not bool), but got {type(start_stage)}"
            assert isinstance(start_T, torch.Tensor), f"start_T must be torch.Tensor, but got {type(start_T)}"
            assert start_T.dtype == torch.float32, f"x dtype must be torch.float32, but got {start_T.dtype}"
            latents = xt
        ...
        for stage_idx in range(start_stage, self.num_stages):
            ...
            if stage_idx == start_stage:
                if start_T.item() == Timesteps[0].item():
                    pass
                else:
                    t_threshold = self.T_to_t_linear(Timesteps, self.scheduler.t, start_T)
                    Timesteps = torch.tensor([start_T] + [i for i in Timesteps if (i - start_T)>=0.001], device = device)
                    if verbose: print(t_threshold)
                    self.scheduler.t = torch.tensor([t_threshold] + [i for i in self.scheduler.t if (i - t_threshold)>=0.001], device = device)
                    if verbose:
                        print("Timesteps:", Timesteps)
                        print("ts:", self.scheduler.t)
            else:
                pass
            if stage_idx == start_stage:
                pass
    ...
    def T_to_t_linear(self, Timesteps, t, T):
        T_end = Timesteps[-1]
        T_start = Timesteps[0]
        t_end = t[-2]
        t_start = t[0]
