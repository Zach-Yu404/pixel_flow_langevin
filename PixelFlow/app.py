...
def inferChangeInit(use_ode_dopri5, noise_shift, cfg_scale, class_label, seed, num_steps_per_stage:list, save_traj = False):
    seed_everything(seed)
    with torch.autocast("cuda", dtype=torch.bfloat16), torch.no_grad():
        samples = pipeline(
            prompt=[class_label] * NUM_EXAMPLES,
            ...
            use_ode_dopri5=use_ode_dopri5,
            save_traj = save_traj,
            save_stage_end_images=True,
            save_stage_end_dir=f"./{args.checkpoint}/stage_end_10step_con",
            save_stage_end_nrow=NUM_EXAMPLES,
        )
    samples = (samples * 255).round().astype("uint8")
    samples = [Image.fromarray(sample) for sample in samples]
    return samples
...
if __name__ == "__main__":
    samples = inferChangeInit(use_ode_dopri5=False, noise_shift=1.0, cfg_scale=1.0, class_label=10, seed=0, 
    num_steps_per_stage = [10 for _ in range(config.scheduler.num_stages)], save_traj =False)
    for idx, sample in enumerate(samples):
        sample.save(f"{args.checkpoint}/10_step_con_sample_{resolution}_{idx}.png")
