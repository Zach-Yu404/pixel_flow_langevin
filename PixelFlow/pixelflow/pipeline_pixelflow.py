...
class PixelFlowPipeline:
    ...
    def __call__(
        ...
        save_traj=False,
        save_stage_end_images=False,
        save_stage_end_dir=None,
        save_stage_end_nrow=4,
    ):
        ...
        save_image_fn = None
        stage_end_dir = None
        if save_stage_end_images:
            from torchvision.utils import save_image as save_image_fn
            if save_stage_end_dir is None:
                save_stage_end_dir = f"./stage_end_images_{resolution}_{self.num_stages}"
            stage_end_dir = os.path.join(save_stage_end_dir, "stage_end")
            os.makedirs(stage_end_dir, exist_ok=True)
        ...
        for stage_idx in range(self.num_stages):
            # print("ts", self.scheduler.t_window_per_stage[stage_idx])
            ...
            if stage_idx > 0:
                ...
                if save_traj:
                    np.save(os.path.join(save_dir,f"noise_stage_{stage_idx}.npy"), noise.cpu().float().numpy())
            ...
            if sample_fn is not None:
                ...
                # import pdb; pdb.set_trace()
                print(Timesteps)
                print(self.scheduler.t)
                
                for T in Timesteps:
                    count+=1
                    # if count%10 ==0:
                    #     print("End: ", T.item(), self.scheduler.t[self.scheduler.step_index].item(), self.scheduler.t[self.scheduler.step_index +1 ].item())
                    # if count%11 ==0:
                    #     print("Start: ",self.scheduler.t[0].item(), self.scheduler.t[1 ].item())
                        # self.scheduler.t[self.scheduler.step_index].item(), self.scheduler.t[self.scheduler.step_index +1 ].item()
                    if save_traj:
                        with torch.no_grad():
                            latents_save = latents.cpu().float().numpy()
                            save_dir = f"./latents_stages_{resolution}_{self.num_stages}_Time_steps{num_inference_steps[stage_idx]}"
                            if not os.path.exists(save_dir):
                                os.makedirs(save_dir+"/mid_states")
                            np.save(os.path.join(save_dir, "mid_states",f"latent_{stage_idx}_{T:08.3f}_{latents_save.shape[-1]}.npy"), latents_save)
                    # import pdb; pdb.set_trace()
            ...
            stage_end = time.time()
            if save_stage_end_images:
                stage_T_end = float(self.scheduler.Timesteps[-1].item())
                save_image_fn(
                    (latents / 2 + 0.5).clamp(0, 1).detach().float().cpu(),
                    os.path.join(
                        stage_end_dir,
                        f"stage{stage_idx:02d}_Tend{stage_T_end:08.3f}_res{latents.shape[-1]}.png",
                    ),
                    nrow=save_stage_end_nrow,
                )
        if save_traj:
            np.save(os.path.join(save_dir, "target.npy"), latents.cpu().float().numpy())
