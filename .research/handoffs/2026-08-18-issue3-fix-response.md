...
**9. 自查：上述修正本身又引入 3 个缺陷，已在 `a8ac308` 修掉**

修完 8 条后我对 `fd57a2a..24907e6` 跑了三路对抗审计（数学 / 配置契约 / 回归，19 条候选
经独立复核后 4 条确认、15 条被驳回）。确认并已修：

- **V9 破坏了 V8**（两路独立发现）：`run_verify` 是单一平坦作用域，V9 重新绑定了
  `si/s_k/e_k/tau`，而其后的 V8 仍复用 V7 的 `x_tau/d_exact/x0_true` → 算子与数据不匹配，
  γ² 探针的 MSE 由 0.0008/0.0072 变成 0.86（高 120 倍），argmin 移到 `4*gamma_true²`。
  因 V8 只 print、不调 `report`，套件照样输出 ALL CHECKS PASSED——**静默错误**。
  已给 V9 独立命名，并让 V8 开头 `report` S1 恒等式（现测 1.110e-15），
  今后再被覆盖会直接 FAIL 而不是印出貌似合理的错数。
- **`--config config.json` 仍会静默读仓库那份**：我用"值 == argparse 默认值"来区分显式与
  默认，而 argparse 无法区分用户显式键入同名值的情况——恰是我声称已删除的旧行为。
  已改为 `default=None`；实测 `config.json` 与 `./config.json` 两种写法现在都读调用方的文件。
- **`tasks_setup.<task>.operator` / `.kw` 完全未校验**：typo 会穿透到
  `build_setup_and_measurement` 自己的 `.get()` 默认值——正是该契约要堵的洞。
  已按任务校验 operator 键集合、`kw` 键须为 `sampler_kw` 子集；实测三类 typo 均被拒。

**10. seed 敏感度（新增证据，影响多条结论的可解释性）**

4 seed（7/42/123/2024，修复后代码）洞区 MSE = 1.0126/1.0276/1.0185/0.9738：
**mean 1.0081、sd 0.0237、极差 5.3%、95% CI 半宽 ±3.7%**；obs 几乎不动（0.00182–0.00184）。
据此：①原"γ² 影响 ~1%"低于噪声地板，已改为不可分辨；②修 √ε 前后同 seed 的
0.9691 vs 1.0276 距均值 1.65 sd，**在噪声内，不能归因于该修复**（先前我表述为"基线平移"，
已更正）；③h₀(3.4×)、K(2.2–2.6×)、anchor(6.2×) 仍远超地板，方向性结论有效。

**未做 / 需评审者注意**：真多步 Langevin 与 S sweep 的实测、oracle 脚本重做尚未进行
（已在 task 记录中标为未完成，不作为论文论据）；anchor 的 6.2× 仍是单 seed。
本机无 GitHub 凭证，本回复由集群侧代发。
