...
### seed 敏感度实测（2026-08-18，新挂载机 GPU，修 √ε 后的代码）

洞区 MSE 到底有多依赖 seed——这是判断"单 seed 结论能不能用"的直接依据：

| seed | 洞区 MSE（trw=0，final-stage last-step） | obs |
|---|---|---|
| 7 | 1.0126 | 0.00183 |
| 42 | 1.0276 | 0.00182 |
| 123 | 1.0185 | 0.00184 |
...
**n=4：mean 1.0081，sd 0.0237，极差 5.3%，95% CI 半宽 ±0.0377（±3.7%）**。
obs 则几乎不动（0.00182–0.00184），说明随机性只作用在洞区——与"洞区链无收缩"的诊断一致。
数据：`results/seed_sens/seed{7,123,2024}/final_metrics.csv` +
`results/debug_box_trw/final_metrics_postfix_sqrt_eps.csv`（seed 42）。

**用法约束（此后所有洞区结论适用）**：

- 效应量 **< 5%** 的差异不可解释，必须多 seed 重复。原"γ² 影响 ~1%"（0.9691 vs 0.9782）
  **低于 seed 噪声**，应改为"γ² 效应无法与 seed 噪声分辨"。
- h₀ 扫描 0.97→3.29（3.4×）、K 扫描 0.97→2.13/2.49（2.2×/2.6×）远超散布，
  方向性结论仍成立（但 K 的语义限于 frozen-score，见上）。
- 修 √ε 前后同 seed 的 0.9691 vs 1.0276 差异（+6%）也落在该分布内
  （0.9691 距均值 1.65 sd），**不能据此说该修复改变了洞区表现**。
- Tweedie anchor 的 1.097→0.178（6.2×，7/7 图）远超散布，是目前唯一确证有效的干预；
  但仍是单 seed，若要写进论文需补多 seed。
