...
ξ₀ 仍是同一次 RNG 抽样。h₀ 用户未给：Alg 2 默认 H0=0.1，另测 0.5、1.0。

## 实现
utils.run_posterior_sampling_alg4 新增 diag_block2_langevin=None（h₀）与 diag_block2_langevin_sign=+1
（默认关闭 → 与 (23) 位级一致；else 分支即原代码）。驱动 scratchpad/block2_langevin.py（junco box、
...
## 结果（results/alg4_block2_langevin/summary.md；seeds 42/43/44 mean±std）
...
| h₀=0.1 符号翻转 | 0.7454±0.0081 | 0.1955±0.0020 | 13.11±0.04 |
| h₀=0.5 符号翻转 | 0.8826±0.0070 | 0.2451±0.0018 | 12.13±0.03 |
...
hole MSE 0.075 优于 3ξ=0 的 0.081）。翻转符号 = x₀ + √h ξ 随机游走，噪声逐 inner 累积 → 0.75–0.88。
我此前"固定 x̂₀ 的平稳律 N(−x̂₀,I)"的担心不成立（x̂₀ 随 x₀ 变）。
诊断定性：它不是 (23) 的 exact draw，x_τ 的条件律被改（更冷 + 样本平均），属于 x_τ 噪声的显式阻尼；
h₀ 最优在 0.5 附近（0.1 欠阻尼且方差大，1.0 略过）。

## 状态
done。代码：utils.py diag_block2_langevin / diag_block2_langevin_sign（默认关，位级不变）。
待用户决定是否把 h₀=0.5 变体设为默认 / 扩到其他 task。
