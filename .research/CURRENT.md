...
- **eq19-endpoint-prior 已按用户「我认为没用，回退」全清**：代码逐字还原、结果目录删、
  回归 0.1447/0.1143/0.1291/0.1495 逐位一致。m=0 零锚数字（0.1227/0.1290）只存 task 文件。
- **utils 新增常驻诊断开关** `diag_noise_off`/`diag_noise_off_from_stage`（draw-then-zero，
  RNG 对齐，默认 None 逐位不变）。定性=诊断上界，非 fix（无 FDT 补偿即篡改条件协方差）。
- **ξ_h=0 探针**（[2,2,1,1]，4 seeds，results/alg4_noise_probe_xih/）：f20 起版（已按用户
  回退，数字留存）spectral 0.0923±0.0084 / pooled 0.0965±0.0047；全程版 0.0940±0.0105 /
  0.0966±0.0045——**均值与 f20 版持平 ⟹ 收益全来自 stage≥2 的 ξ_h**（基线 0.1339/0.1382，
  spread 均 −1/3）。seed42 轨迹两臂四 stage 首次全程单调去噪。
- **进行中**：ξ_h=0 全程 × S_it 扫描（uniform 1/2/5/10 + [2,2,5,5]/[2,2,10,10]，双臂
  4 seeds，检验"无 ξ_h 注入 ⟹ 棘轮死 ⟹ V 形谷底右移"），追加 sit_sweep_results.csv。
