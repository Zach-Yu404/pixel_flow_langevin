...
class PixelFlowScheduler:
    ...
    def step_x0_hat(self, x0_hat, x1_hat, stage_index):
        if self.step_index is None:
            self._step_index = 0
        start_t, end_t = self.start_t[stage_index], self.end_t[stage_index]
        x_e = end_t * x0_hat + (1.0 - end_t) * x1_hat
        # print(pixel_values_end.shape, pixel_values_start.shape, noise.shape)
        x_s = start_t * x0_hat + (1.0 - start_t) * x1_hat
        t_next = self.t[self.step_index + 1].float()
        prev_sample = t_next*x0_hat + (1-t_next)*x1_hat
        self._step_index += 1
        return prev_sample.to(x0_hat.dtype)
    def step_xt_x0_hat(self, x0_hat, x1_hat, stage_index):
        if self.step_index is None:
            self._step_index = 0
        start_t, end_t = self.start_t[stage_index], self.end_t[stage_index]
