...
1. ~~`g_bypass_stage3` 的作用~~ **已解决（2026-08-24）**：用户裁决删除 stage-3 的
   G=I 恒等旁路，`apply_G` 现在在每个 stage 都用真实投影。γ² 双约定测量证实
   real-G 正是网络的训练约定（stage-3 均值 0.0311 vs G=I 的 0.0323，10 个 τ 点
   中 9 个更贴合），旁路本来就是错误约定。stage-3 γ² 已按 real-G 重测
   （`gamma2_meas_alg4.json`）。本节此前基于 G=I 的机制推测作废。
...
- **更正（2026-08-22）**：本文件此前写过"在这个挂载上删除目录会把父目录读坏"。
  **那是一个未经检验的假设，证据不支持，已撤回。** 复查时间线：本会话**最开始**就已经在
  我从未删除过的目录上观察到 EIO（`.research/.runtime`、`.git/logs/refs`、
  baselines 下多个嵌套 `.git`），即 EIO 早于任何删除动作。
  能站得住的说法只有：**该挂载的 readdir 会间歇性返回 EIO，随机命中任意目录**，
  与是否删过东西无关。相应地 `results/alg4/_smoke/`（验证增量落盘/续跑的 scratch，未入 git）
  已按正常清理删除。
- 实际有效的缓解只有两条：`PYTHONDONTWRITEBYTECODE=1`（少一类会被扫描的目录），
  以及**在进程内重试 import**——所有失败都发生在 importlib 的 `_fill_cache`（readdir）阶段，
  进程级重试要重载 2.7GB checkpoint，进程内重试只要毫秒。
- 附带的复现性观察：一次误操作让**三个进程并发**跑了同一配置（同 seed），
  重复格之间各指标最大相对差 **4.3e-4** ——
  四位有效数字一致，与本项目"GPU 非 bit-exact"的既有口径吻合。

## 附记（2026-08-24）：sigma_min=0.39 修复后的全量结果

本目录 CSV 已替换为修复后（sigma_min=0.39, cg_max_iter=300）的运行；
发散版本保留在 git 历史（commit 419fd18 之前）。修复推导见
`../alg4_box_stage3_diagnosis/report.md` §12。

| task | mse（发散版） | mse（修复后） | 改善 | resid | hole |
|---|---:|---:|---:|---:|---:|
| box_inpainting | 0.9217 | **0.0739** | 12.5× | 0.994 | 0.281 |
| random_inpainting | 2.0161 | **0.1209** | 16.7× | 0.999 | 0.171 |
| gaussian_blur | 2.2952 | **0.1657** | 13.9× | 0.993 | — |
| motion_blur | 2.3413 | **0.1612** | 14.5× | 0.977 | — |
| superresolution | 2.3911 | **0.1638** | 14.6× | **0.921** | — |

- **五个任务全部收敛**（stage-3 末 mse 与 stage-2 同量级，无任何爆炸段）；
  Block-1 求解 0/1400 步未收敛；NFE 390→270。
- 五个任务的 measurement residual 全部落到 §8.6 的"趋近 1"
  ——包括此前严重失格的 SR（3.747 → 0.921）。
- b≥1 截止是 schedule 性质（N_k 与任务无关），故一个全局 sigma_min 对五个任务同时成立。
