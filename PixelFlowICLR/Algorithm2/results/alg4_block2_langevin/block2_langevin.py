...
Arms: baseline (exact draw) | h0 in {0.1,0.5,1.0} with the spec sign (+x0_hat) | h0 in {0.1,0.5} with the
flipped sign (-x0_hat, i.e. pull toward +x0_hat) as a labelled secondary check.
...
device = "cuda:0"
ARMS = [("baseline", None, +1.0), ("h0.1", 0.1, +1.0), ("h0.5", 0.5, +1.0), ("h1.0", 1.0, +1.0),
        ("h0.1_flip", 0.1, -1.0), ("h0.5_flip", 0.5, -1.0)]
...
setups = {}
for (name, h0, sign), image, seed in CELLS:
    ...
    if h0 is not None:
        kw["diag_block2_langevin"] = h0; kw["diag_block2_langevin_sign"] = sign
    S2 = dict(S); S2["kw"] = kw
    ...
    json.dump(dict(arm=name, h0=h0, sign=sign, image=image, seed=seed, mse_full=mse_full, mse_hole=mse_hole,
